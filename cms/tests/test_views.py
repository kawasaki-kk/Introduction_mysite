...
class DailyViewTests(TestCase):
    def test_status_in_daily_list_view(self):
        response = self.client.get(reverse('cms:daily_list'), follow=True)
        self.assertEqual(response.status_code, 200)

    def test_status_in_daily_detail_view(self):
        response = self.client.get(reverse('cms:daily_detail', kwargs={"daily_id": 1}), follow=True)
        self.assertEqual(response.status_code, 200)
    """
    def test_status_in_daily_add_view(self):
        response = self.client.get(reverse('cms:daily_add'), follow=True)
        self.assertEqual(response.status_code, 200)
    """
...
class TaskViewTests(TestCase):
    ...
    def test_status_in_task_edit_view(self):
        response = self.client.get(reverse('cms:task_mod'), follow=True)
        self.assertEqual(response.status_code, 200)
    """
