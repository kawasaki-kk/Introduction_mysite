...
class DailyViewTests(TestCase):
    def test_status_in_daily_list_view(self):
        response = self.client.get(reverse('dailyreport:daily_list'), follow=True)
        self.assertEqual(response.status_code, 200)

    def test_status_in_daily_detail_view(self):
        response = self.client.get(reverse('dailyreport:daily_detail', kwargs={"daily_id": 1}), follow=True)
        self.assertEqual(response.status_code, 200)
    """
    def test_status_in_daily_add_view(self):
        response = self.client.get(reverse('dailyreport:daily_add'), follow=True)
        self.assertEqual(response.status_code, 200)
    """
